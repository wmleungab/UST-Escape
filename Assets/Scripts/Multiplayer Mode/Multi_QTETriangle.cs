...
public class Multi_QTETriangle : MonoBehaviour
{
		...
		Debug.Log ("pointx: " + (point.x));
		Debug.Log ("pointx+sizeX: " + (point.x + sizeX));
		Debug.Log ("pointx-sizeX: " + (point.x - sizeX));
		...
		public void checkptTriggerEnter (Collider2D col, string name)
		{
				if (name == "checkPt1" && col.gameObject == ptChild) {
						...
						} else if (!invalidPt_Pos && arriveCP2)
								invalidPt_Pos = true;
				}
				if (name == "checkPt2" && col.gameObject == ptChild) {
						Debug.Log ("ckpt2");
						if (!invalidPt_Pos && arriveCP1)
								arriveCP2 = true;
						else if (!invalidPt_Pos && !arriveCP1)
								invalidPt_Pos = true;
				}
		}
